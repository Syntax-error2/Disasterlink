...
export default function ResponderMobile() {
  ...
  const checkForDispatches = async () => {
    try {
      ...
      const activeIncidents = res.data.filter((i: any) => 
        !['Resolved', 'False Alarm', 'Dismissed'].includes(i.status) &&
        (i.status.includes('Dispatched: ' + user?.name) || i.status === 'Dispatched / Responding')
      );
      
      const newDispatches = activeIncidents.filter((i: any) => !resolvedIds.includes(i.id));
      if (newDispatches.length > 0) {
        const incomingDispatch = newDispatches[0];
        setIncident(incomingDispatch);
        if (status === 'Available') {
          setStatus('Dispatched');
        }
        
        // Only notify if we haven't already shown it (simple dedup)
        if (!toast || !toast.msg.includes(incomingDispatch.incident_type)) {
          showToast(`NEW DISPATCH: ${incomingDispatch.incident_type} at ${incomingDispatch.exact_location}`, 'alert');
              ...
              notifications: [
                {
                  ...
                  body: `You have been dispatched to: ${incomingDispatch.incident_type}`,
                  id: Math.floor(Math.random() * 1000000),
                  schedule: { at: new Date(Date.now() + 500) }
                }
              ]
